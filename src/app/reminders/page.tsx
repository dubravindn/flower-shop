  ...
  return (
    <>
      <PageHeader
        ...
        text="Сохраните день рождения, годовщину или другой повод. Мы заранее напомним и предложим подходящие букеты."
        tone="champagne"
      >
        <ButtonLink
          href={whatsappLink(
            "Здравствуйте! Хочу сохранить важную дату. Событие: . Дата: . Кому: . Напомнить за: дней",
          )}
          size="lg"
        >
          Добавить дату
      ...
      <Section tone="paper-deep">
        ...
        <ul className="flex flex-wrap gap-2">
          {EVENT_TYPES.map((type) => (
            <li
              key={type}
              className="inline-flex min-h-11 items-center rounded-[var(--radius-button)] border border-graphite/20 bg-champagne-paper px-4 text-sm"
            >
              {type}
            </li>
          ))}
        ...
        <div className="mt-10 max-w-[68ch] rounded-[var(--radius-card)] border border-graphite/15 bg-champagne-paper p-6">
          <h3 className="font-sans text-lg font-semibold">Как это работает сейчас</h3>
          <p className="mt-2 text-graphite-muted">
            Форма прямо на сайте появится вместе с защищённым хранилищем: хранить
            телефоны и даты можно только на сервере, с отдельным согласием на
            обработку данных и возможностью отписаться. Пока дату принимает менеджер
            в мессенджере — напоминание придёт так же.
          </p>
          <p className="mt-3 text-sm text-graphite-muted">
            ...
            <a
              href="/consent"
              className="text-red-brand underline underline-offset-4"
            >
