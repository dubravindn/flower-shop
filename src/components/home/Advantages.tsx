    ...
    <Section tone="paper-deep">
      ...
      <ul className="grid gap-px overflow-hidden rounded-[var(--radius-card)] border border-graphite/15 bg-graphite/15 sm:grid-cols-2 lg:grid-cols-3">
        {ADVANTAGES.map((item, index) => (
          <li key={item.title} className="bg-champagne-paper p-6 md:p-7">
            <span
              aria-hidden="true"
              className="mb-4 block font-display text-2xl text-champagne-foil"
            >
              {String(index + 1).padStart(2, "0")}
            </span>
            <h3 className="text-xl">{item.title}</h3>
            <p className="mt-2 text-graphite-muted">{item.text}</p>
          </li>
        ))}
      </ul>
    </Section>
